...
class SeleniumRunner {
  ...
  async runScriptFromCategory(script, isAsync, name) {
    const privileged = (name && name.startsWith('privileged'));

    // Scripts should be valid statements or IIFEs '(function() {...})()' that can run
    // on their own in the browser console. Prepend with 'return' to return result of statement to Browsertime.
    if (isAsync) {
      if (privileged) {
        throw new BrowserError(`Asynchronous privileged JavaScript is not yet supported: cannot run script ${name}`);
      }
      ...
      if (this.options.scriptInput && this.options.scriptInput[name]) {
        args = this.options.scriptInput[name];
      }

      if (privileged) {
        return this.runPrivilegedScript(source, name, args);
      } else {
        return this.runScript(source, name, args);
      }
    }
  }
